 ...
 * @since 1.0.0
 */
public class InnerJoin extends Relation {
	

	public InnerJoin(Class<?> clazz, ACondition... conditions) {
	...
	//self join
	public InnerJoin(Alias alias, ACondition... conditions) {
		this.alias = alias;
	...
	public InnerJoin(boolean beTrue, Class<?> clazz, ACondition... conditions) {
		if (beTrue) {
	...
	public String toSQL() {
		...
		s.append(SQLConsts.SQL_INNER_JOIN).append(tableName).append(SQLConsts.SQL_ON);
