...
public class POW extends Expression {
    ...
    public <T> POW(SFunction<T, ?> propertyFunction, Number value) {
        this.fieldPart = new Field<>(propertyFunction);
        this.operator = Operator.POW.getOp();
        this.value = value;
    }
    
    public <T> POW(Alias alias, Number value) {
    ...
    @Override
    public String toSQL() {
        return InnerHelper.format(SQLConsts.FUN_POW, fieldPart.toSQL(), value);
    }
}
