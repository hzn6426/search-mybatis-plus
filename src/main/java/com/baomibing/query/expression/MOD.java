...
public class MOD extends Expression {
    ...
    public <T> MOD(SFunction<T, ?> propertyFunction, Number value) {
        this.fieldPart = new Field<>(propertyFunction);
        this.operator = Operator.MOD.getOp();
        this.value = value;
    }
    
    public <T> MOD(Alias alias, Number value) {
    ...
    @Override
    public String toSQL() {
        return InnerHelper.format(this.fieldPart.toSQL(), value);
    }
}
