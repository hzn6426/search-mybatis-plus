 ...
 * @since: 1.0.0
 */
public class MULTIPL extends Expression {


    public <T> MULTIPL(SFunction<T, ?> propertyFunction, Number value) {
        this.fieldPart = new Field<>(propertyFunction);
        this.operator = Operator.MUL.getOp();
        this.value = value;
    }
    
    public <T> MULTIPL(Alias alias, Number value) {
    ...
    @Override
    public String toSQL() {
        return this.fieldPart.toSQL() + OP_MULTIPL + value;
    }
}
