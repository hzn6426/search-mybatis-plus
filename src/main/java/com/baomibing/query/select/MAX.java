...
public class MAX extends SQLFunction {
	...
	public MAX(String sql) {
		this.sql = sql;
	}
	

	public <T1> MAX(SFunction<T1, ?> fun) {

		this(new Field<>(fun));
	}

	
	private MAX(QueryPart... parts) {
		if (fields == null) {
	...
	public String toSQL() {
		return InnerHelper.format(SQLConsts.FUN_MAX, StringUtils.isNotBlank(sql) ? sql : this.fields.stream().map(f -> f.toSQL()).collect(Collectors.joining(",")));
	}
}
