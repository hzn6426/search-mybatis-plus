...
public class MIN extends SQLFunction {
	...
	public MIN(String sql) {
		this.sql = sql;
	}
	

	public <T1> MIN(SFunction<T1, ?> fun) {

		this(new Field<>(fun));
	}

	
	private MIN(QueryPart... parts) {
		if (fields == null) {
	...
	public String toSQL() {
		return InnerHelper.format(SQLConsts.FUN_MIN, StringUtils.isNotBlank(sql) ? sql : this.fields.stream().map(f -> f.toSQL()).collect(Collectors.joining(",")));
	}
}
